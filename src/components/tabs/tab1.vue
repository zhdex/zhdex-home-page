...
export default {
  ...
  mounted() {
    ...
    if(zhdexdata){
        this.color = zhdexdata.color;
        this.brightness = zhdexdata.brightness;
        this.blur = zhdexdata.blur;
    }else{
  ...
  methods: {
    ...
    submitdata() {      
        ...
        setTimeout(() => {
            this.loading = false;
            // 注意数字String格式化
            this.setCookie('zhdexdata', {color:this.color,brightness:String(this.brightness),blur:String(this.blur),backgroundblur:String(this.backgroundblur)},0.005);
            location.reload();
        }, 800)   
    },
    redefault(){              
